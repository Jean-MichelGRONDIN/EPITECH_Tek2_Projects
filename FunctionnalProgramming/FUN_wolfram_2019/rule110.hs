...
get_this_one x [] = True
get_this_one x tab = myOr (myXor (myNth tab x) (myNth tab (x + 1))) (myAnd (myNth tab x) (myNot (myNth tab (x - 1))))
...
rule110 :: Int -> Int -> Int -> Int -> Int -> [Bool] -> IO ()
rule110 n linemax start window move tab
    ...
    | otherwise = do if start > linemax then (if (start) <= (n - 2) && (n - 2) <= (start + linemax) then print_that_line n linemax window move tab else putStr "")
                     ...
                     rule110 (n + 1) linemax start window move (creat_tab n 1 tab)
