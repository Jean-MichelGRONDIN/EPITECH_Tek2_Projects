...
get_this_one x [] = True
get_this_one x tab = myXor (myNth tab (x - 1)) (myOr (myNth tab x) (myNth tab (x + 1)))
...
rule30 :: Int -> Int -> Int -> Int -> Int -> [Bool] -> IO ()
rule30 n linemax start window move tab
    ...
    | otherwise = do if start > linemax then (if (start) <= (n - 2) && (n - 2) <= (start + linemax) then print_that_line n linemax window move tab else putStr "")
                     ...
                     rule30 (n + 1) linemax start window move (creat_tab n 1 tab)

-- n commence a 1
-- x commence a 1
-- tab = [] au debut

-- 1 + 2 * line = nb d'elem par ligne